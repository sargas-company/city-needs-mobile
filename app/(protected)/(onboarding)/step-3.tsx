...
const Step3 = () => {
    ...
    const handleFinish = async () => {
        ...
        setError(null)
        try {
            await dispatch(authApi.endpoints.authSync.initiate({ onboardingStep: 0 }, { forceRefetch: true })).unwrap()
    ...
    return (
        <SafeAreaView className="flex-1 bg-white px-6">
            <View className="flex-1 items-center justify-center gap-4">
                <Text className="text-2xl font-bold text-black">Onboarding Step 3</Text>
                <Text className="text-base text-gray-700">{`Hello ${profileUser?.username ?? ''}, final step.`}</Text>
                ...
                >
                    <Text className="text-base font-semibold text-white">{loading ? 'Please wait...' : 'Finish'}</Text>
                </Pressable>
            </View>
        </SafeAreaView>
    )
